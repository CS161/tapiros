...
int sys_open(char* pathname, int flags, int *retval) {
	...
	err = vfs_open(pathname, flags, 0666, &vf->vf_vnode);	// 0666 for read/write (and as tribute to Satan)
	if(err != 0) {											// vf_flags will enforce perms
		goto err3;
	}

	err = add_vfile(vf, fd);	// add the appropriate entries to the per-process
	if(err != 0) {				// and global file descriptor tables
		goto err4;
	}		
